...
const StatisticInfoTU = () => {
  ...
  const answerAPI =  (response) => {
      ...
      if(response.status === '200'){            
          if(typeof(response) === 'object'){
              let objectAPI = response.message[7];
  ...
  if(apiJson){
    if(apiJson.req_work){
        arrayStat.push({
            name: 'В работе',
            population: (apiJson.req_work ? Number(apiJson.req_work) : 0 ),
            color: '#1B78AF',
    ...
    if(apiJson.req_otaz){
        arrayStat.push({
            name: 'Отказ',
            population: (apiJson.req_otaz ? Number(apiJson.req_otaz) : 0 ),
            color: '#f09732',
            legendFontColor: 'black',
            legendFontSize: 13,
        })
    ...
    if(apiJson.req_arhive){
        arrayStat.push({
            name: 'В архиве',
            population: (apiJson.req_arhive ? Number(apiJson.req_arhive) : 0 ),
            color: '#cfcfcf',
            legendFontColor: 'black',
            legendFontSize: 13,
        })
    ...
    <View style={styles.card}>
        { 
            ...
            (<View style={styles.wrapper}>
                <Text style={styles.header}> Статистика Инфо.ТУ </Text>
                <PieChart
                    data={arrayStat}
...
export default StatisticInfoTU